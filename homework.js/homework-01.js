//Task-1

console.log(' I start to practice "JavaScript" today, and i like it.');
console.log(' The secret of getting ahead is getting started.');
console.log(" \"Don't limit yourself.\" ");
console.log(' Invest in your dreams. Grind now. Shine later.');
console.log(" It's not the load that breaks you down, it is the way you carry it.");
console.log(' The hard days are what makes you stronger.');
console.log(' You can waste you lives drawing lines. Or can live your life crossing them');

//Task-2

console.log(' \tJavaScript is a high level programming language \nprimarily used to build web applications.It is not limited\nto the web; it can also be used for the backned\ndevolepment with technologies like Node.Js.\n\tIt is open source, and has huge community support,\nwhich means there are plenty of resources and support\navailable for learning. It uses a syntax like other\nprogramming languages and easy to learn. ');

// Task-3


 let myAge = 41;
 let myFavoriteNumber = 'Blue';
 let myHeight = 160;
 let myWeight = 48;
 let myFavoriteLetter = 'A'

 console.log (myAge);
 console.log (myFavoriteNumber);
 console.log (myHeight);
 console.log (myAge);
 console.log (myFavoriteLetter);

 // Task-4

 let num1 = 25, num2 = 35;
 console.log('The sum of the numbers = ' + (num1 + num2));

 // Task-5

 let n1 = 5, n2 = 7;
 
 console.log('The product of the numbers = ' + n1 * n2);
 
 
 // Task-6

 let number1 = 24, number2 = 10;
 console.log('The sum of the numbers is = ' + (number1 + number2));
 console.log('The product of the numbers is = ' + number1 * number2);
 console.log('The subtraction of the number is =' + (number1 - number2));
 console.log('The division of the numbers is =' + number1 / number2);
 console.log('The remainder of the numbers is =' + number1 % number2);

 // Task-7

 const i1 = 7, i2 = 11;
 console.log('The average of the numbers is =' + (i1 + i2)/ 2);

 // Task-8

 let a1 = 6, a2 = 10, a3 = 12, a4 = 15, a5 = 17;
 console.log('The avarage of the numbers is = ' + (a1 + a2 + a3 + a4 + a5)/ 5);

 // Task-9

 let b1 = 5,  b2 = 6,  b3 = 10;
 console.log('The 5 multiplied with 5 is = ' + (b1**2));
 console.log('The 6 multiplied with 6 is = '+ (b2**2));
 console.log('The 10 multiplied with 10 is = '+ (b3**2));

 // Task-10

 let side = 7 
 console.log('The perimeter of the square = ' + side * 4);
 console.log('The area of the square = ' + (side**2));


 // Task-11

 let favBook = 'JS Algorithms & Data Structures';
 let favColor = 'Blue';
 let favNumber = 7;
  
 console.log('The favorite book is '+ favBook , '\nThe favorite color ' + favColor  , '\nThe favorite number is ' + favNumber);


 // Task-12

 let firstName = 'John';
 let lastName = 'Doe';
 let age = 45;
 let emailAddress = 'johndoe@gmail.com';
 let phoneNumber = '(123) 123 1234';
 let address = '123 St Chicago IL 12345';

 console.log('User who joined this program is ' + firstName , lastName +'.' + " John's age is " + age +'.', "John's \nemail address is " + emailAddress,'.\nphonenumber is ' + phoneNumber +"," +' and address is\n' + address);




 
