...
const classicBooks = bookstore
  .filter((books) => books.genre === "Classic")
  .map((book) => book.title);
console.log(classicBooks);

// Group 2 Solutions

const cheap = bookstore.reduce((acc, curr) => {
  let cheapest = bookstore[0].price;

  if (cheapest > curr.price) {
    cheapest = curr.price;
    acc = curr;
  }
  return acc;
...
const most = bookstore.reduce((acc, curr) => {
...
const classic = bookstore.reduce((acc, curr) => {
...
// Find all the items in the cart with their quantity -> [ 'Laptop, 1', 'Smartphone 2', 'Headphones, 3' ]
const itemQuantities = shoppingCart.items.map((item) =>
